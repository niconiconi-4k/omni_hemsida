...
- 首页导航栏顺序调整，应用场景区提前。
- Banner高度压缩，轮播图高度提升，图片自适应填充，切换速度更慢（5秒）。
- 特点区重构为马赛克拼接风格，色块错落分布，深色块内文字为白色，小标题为淡紫色。
- 客户评价区背景支持自定义图片，立绘支持透明PNG，底部紧贴框底，背景色为主紫色。
- 新增 404 错误页 `pages/error404.html`，风格与 coming-soon.html 一致，支持多语言。
- 多语言词条补充 features.sectionDesc，404相关内容。
- 响应式布局优化，适配桌面与移动端。
- README.md 按需同步更新。
